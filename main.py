...
if (selected2 == 'Terapan') : 
    ...
    #sidebar
    with st.sidebar :
        selected = option_menu ('Pilih',['Game Tebak Angka','Groupby Data Excel',],
        default_index=0)
    if(selected == 'Game Tebak Angka') :
        st.title('Tebak Angka')
        st.text('')
        acak = random.randint(1,10)
        data = (st.number_input("masukan angka (1-10): ",0))
        mulai = st.button('mulai')
        if data == acak:
            st.write("tebakan benar", data)
        if data != acak:
            st.write('tebakan salah')
    ...
    if(selected == 'Groupby Data Excel') :
        ...
        st.markdown('https://docs.google.com/spreadsheets/d/1ZNMUBQ0RV4F6e8ArO8C-36qOkhtgTqq5/edit?usp=sharing&ouid=102979557255368654542&rtpof=true&sd=true')
